 ...
 * 2. EDIT THE OBJECTIVE FUNCTION -- (THE functions FUNCTION)
...
int currentIter{0};
const int population{10}; // usually depicts number of wolves...
const int maxiter{500};   // refers to the number of iterations
    ...
    for (int i{1}; i <= maxiter; i++)
    {
        ...
        */
        ...
        for (int i{1}; i <= population; i++)
        {
            ...
            if (previous > latest)
            { // greedy selection agreees for the Swap...
...
int main(int argc, char *argv[])
{
    ...
    vector<vector<double>> bounds{{0.575, 0.892}, {40, 60}, {2, 6}};
